...
List of ~elisp_library~ dependencies.
Optional.
...
Run Emacs in interactive instead of batch mode.
Optional.
...
Texinfo manual file to write; must end in ~.texi~.
Mandatory.
...
Org-mode file to use as manual source; must end in ~.org~.
Mandatory.
...
Execution requirements for compilation and test actions.
Optional.
...
Whether actions should inherit the external shell environment.
Optional.
