...
git_override(
    ...
    commit = "99766fa34aa306106843393fe54a80f2f5880ced",
)
#+END_SRC
...
http_archive(
    ...
    integrity = "sha384-qUzwjALUk4DBEGCCEEMQxD3eVz6nsnsflBBlpAZpvF5sirVGQO4xy7Zcz4hL02NZ",
    strip_prefix = "rules_elisp-99766fa34aa306106843393fe54a80f2f5880ced",
    urls = [
        "https://github.com/phst/rules_elisp/archive/99766fa34aa306106843393fe54a80f2f5880ced.zip",  # 2023-12-03
    ],
)
...
#+TEXINFO: @noindent
See [[https://bazel.build/external/overview#workspace-system][the Bazel
documentation on WORKSPACE dependencies]] for background information.  Since
~rules_elisp~ depends on ~rules_python~, you’ll also need to add a dependency on
the latter.  See
[[https://rules-python.readthedocs.io/en/latest/getting-started.html#using-a-workspace-file][the
...
~examples~ directory for details.  Note that the C++ code used by ~rules_elisp~
requires at least C++14, but Bazel still compiles with C++11 by default.  See
