...
#+TEXINFO: @noindent
Since ~rules_elisp~ depends on ~rules_python~, you’ll also need to add a
dependency on the latter.  See
[[https://rules-python.readthedocs.io/en/latest/getting-started.html#using-a-workspace-file][the
~rules_python~ documentation]].  Then you can use the ~elisp_library~,
...
Note that the C++ code used by ~rules_elisp~ requires at least C++14, but Bazel
