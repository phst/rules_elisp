...
// version check to ensure that both emacs-module.h and the running Emacs binary are recent enough, using this pattern:
//
...
#endif
  // Try again with a small integer.
  intmax_t i = ExtractInteger(ctx, value);
  if (i < 0) {
    ArgsOutOfRange(ctx, value, MakeInteger(ctx, 0),
                   MakeUInteger(ctx, UINTMAX_MAX));
...
/*
// Adopts multiple generalized sequences using AdoptSequence.  The ‘values’ and
// ‘arrays’ arrays must both be of length ‘count’.  Fills in ‘arrays’.
static bool AdoptSequences(struct Context ctx, upb_Arena* arena,
                           const upb_FieldDef* type, ptrdiff_t count,
                           emacs_value* values, const upb_Array** arrays) {
  for (ptrdiff_t i = 0; i < count; ++i) {
    const upb_Array* array = AdoptSequence(ctx, arena, type, values[i]);
...
// Concatenates multiple arrays into a single new array.  The source arrays must
// all have the same type.
static upb_Array* ConcatenateArrays(struct Context ctx, upb_Arena* arena,
                                    const upb_FieldDef* type, ptrdiff_t count,
                                    const upb_Array* const* srcs) {
  size_t length = 0;
  for (ptrdiff_t i = 0; i < count; ++i) {
    if (AddOverflowSize(ctx, length, upb_Array_Size(srcs[i]), &length)) {
      return NULL;
    }
  }
  upb_Array* dest = NewArray(ctx, arena, type, length);
  if (dest == NULL) return NULL;
  size_t k = 0;
  for (ptrdiff_t i = 0; i < count; ++i) {
    const upb_Array* src = srcs[i];
    size_t size = upb_Array_Size(src);
    for (size_t j = 0; j < size; ++j) {
      upb_Array_Set(dest, k, upb_Array_Get(src, j));
      ++k;
    }
  }
  assert(k == length);
...
static struct RangeArg ExtractRange(struct Context ctx, size_t size,
  ...
  if (nargs == 2) {
    ...
    if (IsNotNil(ctx, arg)) to = ExtractRangedInteger(ctx, -ssize, ssize, arg);
  }
  if (!Success(ctx)) return null;
...
static void RegisterFileDescriptors(
  ...
  for (size_t i = 0; i < count; ++i) {
    upb_Status status;
    upb_Status_Clear(&status);
    if (upb_DefPool_AddFile(MutableDefPool(ctx), files[i], &status) == NULL ||
...
static emacs_value PrintMessage(emacs_env* env,
                                ptrdiff_t nargs ABSL_ATTRIBUTE_UNUSED,
                                emacs_value* args, void* data) {
  struct Context ctx = {env, data};
  assert(nargs == 2);
...
/*
static emacs_value Concatenate(emacs_env* env, ptrdiff_t nargs,
                               emacs_value* args, void* data) {
  struct Context ctx = {env, data};
  assert(nargs >= 1);
  const upb_FieldDef* type = NULL;
  struct LispArena arena;
  for (ptrdiff_t i = 0; i < nargs; ++i) {
    emacs_value arg = args[i];
    if (Predicate(ctx, kArrayP, arg)) {
      struct ArrayArg array = ExtractArray(ctx, arg);
      if (array.type == NULL) return NULL;
      type = array.type;
      arena = array.arena;
      break;
  ...
  if (type == NULL) {
    WrongTypeArgument(ctx, kArrayListP, List(ctx, nargs, args));
    return NULL;
  }
  const upb_Array** srcs = AllocateArray(ctx, (size_t)nargs, sizeof *srcs);
  if (srcs == NULL) return NULL;
  upb_Array* dest = AdoptSequences(ctx, arena.ptr, type, nargs, args, srcs)
                        ? ConcatenateArrays(ctx, arena.ptr, type, nargs, srcs)
                        : NULL;
  free(srcs);
  return dest == NULL ? NULL : MakeMutableArray(ctx, arena, type, dest);
}
*/
...
static emacs_value PrintArray(emacs_env* env,
                              ptrdiff_t nargs ABSL_ATTRIBUTE_UNUSED,
                              emacs_value* args, void* data) {
  struct Context ctx = {env, data};
  assert(nargs == 2);
...
static emacs_value SortArray(emacs_env* env,
  ...
  struct Context ctx = {env, data};
  assert(nargs == 2);
  emacs_value pred = args[0];
  struct MutableArrayArg array = ExtractMutableArray(ctx, args[1]);
  if (array.type == NULL) return NULL;
  size_t size = upb_Array_Size(array.value);
...
static emacs_value MapGet(emacs_env* env, ptrdiff_t nargs, emacs_value* args,
  ...
  upb_MessageValue key = AdoptScalar(ctx, map.arena.ptr, type.key, args[1]);
  if (!Success(ctx)) return NULL;
  emacs_value default_val = nargs == 3 ? args[2] : Nil(ctx);
  upb_MessageValue value;
...
static emacs_value PrintMap(emacs_env* env,
                            ptrdiff_t nargs ABSL_ATTRIBUTE_UNUSED,
                            emacs_value* args, void* data) {
  struct Context ctx = {env, data};
  assert(nargs == 2);
...
int VISIBLE emacs_module_init(struct emacs_runtime* rt) {
  ...
  Defun(ctx, "elisp/proto/print-message", 2, 2,
  ...
  Defun(ctx, "elisp/proto/print-array", 2, 2,
  ...
  Defun(ctx, "elisp/proto/print-map", 2, 2,
