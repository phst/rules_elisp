...
using rules_elisp::CastNumber;
using rules_elisp::CommonOptions;
using rules_elisp::ContainsNull;
using rules_elisp::Encoding;
using rules_elisp::Environment;
using rules_elisp::ExecutableKind;
using rules_elisp::FileExists;
using rules_elisp::IsAbsolute;
using rules_elisp::kSeparator;
using rules_elisp::kWindows;
using rules_elisp::LoadPathArgs;
using rules_elisp::ManifestFile;
using rules_elisp::NativeChar;
using rules_elisp::NativeString;
using rules_elisp::NativeStringView;
using rules_elisp::PercentEncode;
using rules_elisp::Run;
using rules_elisp::Runfiles;
using rules_elisp::ToNarrow;
using rules_elisp::ToNative;
using rules_elisp::ToolchainMode;

static absl::StatusOr<NativeString> Quote(const NativeStringView arg) {
...
static absl::StatusOr<int> RunTest(
    const std::string_view source_repository, const CommonOptions& opts,
    const NativeStringView runfiles_elc, const NativeStringView run_test_elc,
    const absl::Span<const NativeString> skip_tests,
    const absl::Span<const NativeString> skip_tags, bool module_assertions,
  ...
  if (module_assertions) {
    args.push_back(RULES_ELISP_NATIVE_LITERAL("--module-assertions"));
  }
  ...
  args.push_back(RULES_ELISP_NATIVE_LITERAL("--load=") +
  ...
  for (const NativeString& file : opts.load_files) {
    ...
    const absl::StatusOr<NativeString> quoted = Quote(*abs_name);
    if (!quoted.ok()) return quoted.status();
    args.push_back(RULES_ELISP_NATIVE_LITERAL("--test-source=") + *quoted);
  }
  for (const NativeString& test : skip_tests) {
    const absl::StatusOr<NativeString> quoted = Quote(test);
    if (!quoted.ok()) return quoted.status();
    args.push_back(RULES_ELISP_NATIVE_LITERAL("--skip-test=") + *quoted);
  }
  for (const NativeString& tag : skip_tags) {
    const absl::StatusOr<NativeString> quoted = Quote(tag);
    if (!quoted.ok()) return quoted.status();
    args.push_back(RULES_ELISP_NATIVE_LITERAL("--skip-tag=") + *quoted);
  }

  args.push_back(RULES_ELISP_NATIVE_LITERAL("--"));

  if (!original_args.empty()) {
    for (auto it = std::next(original_args.cbegin());
         it != original_args.cend(); ++it) {
     const absl::StatusOr<NativeString> quoted = Quote(*it);
    if (!quoted.ok()) return quoted.status();
...
(int argc, NativeChar** argv) {
  ...
  while (argc > 0) {
    ...
    if (ConsumePrefix(arg, RULES_ELISP_NATIVE_LITERAL("--wrapper="))) {
      ...
      LOG(FATAL) << absl::StrFormat("Invalid command-line argument %s", arg);
    }
  }

  const absl::FixedArray<NativeStringView> original_args(argv, argv + argc);

  const absl::StatusOr<int> code =
      RunTest(BAZEL_CURRENT_REPOSITORY, opts, runfiles_elc, run_test_elc,
              skip_tests, skip_tags, module_assertions, original_args);
  if (!code.ok()) {
