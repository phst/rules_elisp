...
using rules_elisp::MakeAbsolute;
using rules_elisp::MakeRelative;
...
static absl::StatusOr<int> RunBinary(
    const std::string_view source_repository, const CommonOptions& opts,
    const NativeStringView runfiles_elc, const bool interactive,
    std::vector<int> input_args, std::vector<int> output_args,
  ...
  const absl::StatusOr<std::string> narrow_elc =
      ToNarrow(runfiles_elc, Encoding::kAscii);
  if (!narrow_elc.ok()) return narrow_elc.status();
  const absl::StatusOr<std::vector<NativeString>> load_path_args =
      LoadPathArgs(*runfiles, opts.load_path, *narrow_elc);
  ...
  const absl::StatusOr<std::vector<NativeString>> input_files =
      ArgFiles(original_args, runfiles_dir, std::move(input_args));
  if (!input_files.ok()) return input_files.status();
  const absl::StatusOr<std::vector<NativeString>> output_files =
      ArgFiles(original_args, runfiles_dir, std::move(output_args));
...
[[nodiscard]] static int ParseInt(const NativeStringView arg) {
  const absl::StatusOr<std::string> narrow = ToNarrow(arg, Encoding::kAscii);
  CHECK_OK(narrow) << absl::StrFormat("Invalid integer argument %s", arg);
  int i;
  const bool ok = absl::SimpleAtoi(*narrow, &i);
  CHECK(ok) << "Cannot parse integer argument " << narrow;
  return i;
}
...
(int argc, NativeChar** argv) {
  ...
  while (argc > 0) {
    ...
    if (ConsumePrefix(arg, RULES_ELISP_NATIVE_LITERAL("--wrapper="))) {
      ...
      input_args.push_back(ParseInt(arg));
    } else if (ConsumePrefix(arg,
                             RULES_ELISP_NATIVE_LITERAL("--output-arg="))) {
      output_args.push_back(ParseInt(arg));
    } else {
  ...
  const absl::StatusOr<int> code =
      RunBinary(BAZEL_CURRENT_REPOSITORY, opts, runfiles_elc, interactive,
                std::move(input_args), std::move(output_args), original_args);
  if (!code.ok()) {
